...
document
  .querySelector("#btnFixImage")
  .addEventListener("click", imageSelection);

function imageSelection() {
  let playerOne = document.querySelector("#slcPlayerOne").value;
  let playerTwo = document.querySelector("#slcPlayerTwo").value;
  if (playerOne === "disabled" || playerTwo === "disabled") {
    textField.innerHTML = `Por favor elija una imagen para los dos jugadores`;
    return 0;
  }
  if (playerOne === playerTwo) {
    textField.innerHTML = `Imagen de jugador 1 no puede ser igual a la de jugador 2`;
    return 0;
  }

  firstPlayerIMG = playerOne;
  secondPlayerIMG = playerTwo;
  document.querySelector("#playerSelection").setAttribute("hidden", "hidden");
  document.querySelector("#tableTaTeTi").removeAttribute("hidden");
  textField.innerHTML = "";
}
